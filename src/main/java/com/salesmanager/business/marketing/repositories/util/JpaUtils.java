package com.salesmanager.business.marketing.repositories.util;
...
public abstract class JpaUtils {
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static void initializeRelation(EntityManager em, Object entity, int depth, String attrName) {
		// return on nulls, depth = 0 or already initialized objects
		if (entity == null || depth == 0) {
			return;
		}
		...
		if (id != null) {
			...
			if (!unitUtil.isLoaded(entity, attribute.getName())) {
				if (attribute.isCollection()) {
					intializeCollection(em, entity, attached, attribute, depth);
				} else if (attribute.isAssociation()) {
					intialize(em, entity, attached, attribute, depth);
				}
			}
